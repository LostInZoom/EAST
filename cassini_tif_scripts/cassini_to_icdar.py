...
W, H = 512, 512 # 1280, 720 # taille de l'imagette à découper
...
for x in range(0, width, int(W * (1 - OVERLAP))):
    ...
    for y in range(0, height, int(H * (1 - OVERLAP))):
        ...
        with rasterio.open(f'{output_path}/{root_name}.tif', 'w', driver='GTiff', width=W, height=H, count=4, dtype=w.dtype) as dst:
            dst.write(w)

        # write label in icdar format : bottom_left, bottom_right, top_right, top_left
        label_filename = f'{output_path}/gt_{root_name}.txt'
        txt = []
